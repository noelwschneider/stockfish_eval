...
export default function scale_factor(pos, eg) {
    ...
    let sf = 64;
    let pc_w = pawn_count(pos_w), pc_b = pawn_count(pos_b);
    let qc_w = queen_count(pos_w), qc_b = queen_count(pos_b);
    let bc_w = bishop_count(pos_w), bc_b = bishop_count(pos_b);
    let nc_w = knight_count(pos_w), nc_b = knight_count(pos_b);
    let npm_w = non_pawn_material(pos_w), npm_b = non_pawn_material(pos_b);
    let bishopValueMg = 825, bishopValueEg = 915, rookValueMg = 1276;
